...
class Perceptron:
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.n_iterations):
            ...
            if (epoch + 1) % 10 == 0:
                print(f"Época {epoch + 1:3d} | Erros: {errors:2d} | Pesos: {self.weights} | Bias: {self.bias:.4f}")
...
# ==========================================
# Parte 3.3 Visualizando a Fronteira de Decisão
# ==========================================
def plot_decision_boundary(X, y, perceptron, title):
    ...
    Matematicamente: w1*x1 + w2*x2 + b = 0
    """
    # Configurar o gráfico
    plt.figure(figsize=(10, 7))

    # Plotar pontos de dados
    # Classe 0 (circulos vermelhos)
    plt.scatter(X[y==0,0], X[y==0,1], color='red', marker='o', s=200, edgecolors='black', linewidths=2, label='Classe 0', alpha=0.7)
    # Classe 1 (estrelas verdes)
    plt.scatter(X[y==1,0], X[y==1,1], color='green', marker='*', s=400, edgecolors='black', linewidths=2, label='Classe 1', alpha=0.7)

    # Calcular e plotar fronteira de decisão
    # A fronteira é definida por: w1*x1 + w2*x2 + b = 0
    # Resolvendo para x2: x2 = -(w1*x1 + b) / w2

    # Criar pontos x1 de -0.5 a 1.5 (margem ao redor dos dados)
    x1_boundary = np.linspace(-0.5, 1.5, 100)

    # Calcular x2 correspondentes
    ...
    plt.xlim(-0.5, 1.5)
    plt.ylim(-0.5, 1.5)
    plt.xlabel('x1 (Entrada 1)', fontsize=14, fontweight='bold')
    plt.ylabel('x2 (Entrada 2)', fontsize=14, fontweight='bold')
    ...
    equation = f'Fronteira: {perceptron.weights[0]:.2f}x1 + {perceptron.weights[1]:.2f}x2 + {perceptron.bias:.2f} = 0'
    plt.text(0.02, 0.98, equation, transform=plt.gca().transAxes, fontsize=11, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

    plt.tight_layout()
    plt.show()

# ==========================================
# Parte 3: Testando com o Problema AND
# ==========================================
print("\n" + "="*60)
...
for i in range(len(X_and)):
    print(f"Entrada: {X_and[i]} | Esperado: {y_and[i]} | Previsto: {predictions_and[i]} | {' CORRETO' if y_and[i] == predictions_and[i] else 'X ERRADO'}")
...
plt.plot(range(1, len(perceptron_and.errors_history) + 1), perceptron_and.errors_history, marker='o', linewidth=2, markersize=8, color='blue')
...
print(f". Convergência: {'SIM ' if perceptron_and.errors_history[-1] == 0 else 'NÃO X'}")
...
# ==========================================
# Parte 4: Testando com o Problema OR
# ==========================================
print("\n" + "="*60)
...
for i in range(len(X_or)):
    print(f"Entrada: {X_or[i]} | Esperado: {y_or[i]} | Previsto: {predictions_or[i]} | {' CORRETO' if y_or[i] == predictions_or[i] else 'X ERRADO'}")
...
# ==========================================
# Parte 5: A Limitação do Perceptron - Problema XOR
# ==========================================
print("\n" + "="*60)
...
print("\n ATENÇÃO: XOR NÃO é linearmente separável!")
...
for i in range(len(X_xor)):
    print(f"Entrada: {X_xor[i]} | Esperado: {y_xor[i]} | Previsto: {predictions_xor[i]} | {' CORRETO' if y_xor[i] == predictions_xor[i] else 'X ERRADO'}")
...
plt.figure(figsize=(10, 6))
plt.plot(range(1, len(perceptron_xor.errors_history) + 1), perceptron_xor.errors_history, marker='o', linewidth=2, markersize=8, color='red')
plt.xlabel('Época', fontsize=14, fontweight='bold')
plt.ylabel('Número de Erros', fontsize=14, fontweight='bold')
plt.title('Curva de Aprendizado XOR (Não Converge)', fontsize=16, fontweight='bold')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
# ==========================================
# Parte 6: Experimentando com Taxa de Aprendizado
# ==========================================
print("\n" + "="*60)
...
# ==========================================
# 6.2 Análise Comparativa Detalhada
# ==========================================
print("\n" + "="*60)
...
print("\n{:<10} {:<10} {:<12} {:<15}".format("Taxa (η)", "Épocas", "Convergiu?", "Erros Finais"))
print("-" * 60)
